...
VkCommandBuffer VulkanUtils::beginRecordingCommands() const
{
    ...
    allocInfo.commandPool = _vulkanInstance->getCommandPool();
...
void VulkanUtils::endRecordingAndSubmitCommands(VkCommandBuffer commandBuffer) const
{
    ...
    // Free command buffer
    vkFreeCommandBuffers(_vulkanInstance->getLogicalDevice(), _vulkanInstance->getCommandPool(), 1, &commandBuffer);
}
